...
def run_inference(
    features, encoder, full_sequence=myconfig.USE_FULL_SEQUENCE_INFERENCE
):
    """Get the embedding of an utterance using the encoder."""
    if full_sequence:
        # Full sequence inference.
        batch_input = (
            torch.unsqueeze(torch.from_numpy(features), dim=0)
            .float()
            .to(myconfig.DEVICE)
        )
        ...
        if not sliding_windows:
            return None
        batch_input = (
            torch.from_numpy(np.stack(sliding_windows)).float().to(myconfig.DEVICE)
        )
        batch_output = encoder(batch_input)
...
class TripletScoreFetcher:
    ...
    def __call__(self, i):
        ...
        # neg_vector = neg_embedding.tolist()
        if (
    ...
    with ThreadPool(myconfig.NUM_PROCESSES) as pool:
        while num_eval_triplets > len(labels) // 2:
            label_score_pairs = pool.map(
                fetcher, range(len(labels) // 2, num_eval_triplets)
            )
